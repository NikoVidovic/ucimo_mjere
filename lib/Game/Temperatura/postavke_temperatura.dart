...
import 'package:hello_world/Game/Temperatura/temperatura_zadatci_button.dart';
import 'package:provider/provider.dart';
import '../../app_state.dart';

class PostavkeTemeperatura extends StatefulWidget {
  final void Function(int) switchToTab;
  const PostavkeTemeperatura({
    required this.switchToTab,
    super.key,
  });

  @override
  State<PostavkeTemeperatura> createState() => _PostavkeTemperaturaState();
}

class _PostavkeTemperaturaState extends State<PostavkeTemeperatura> {
  ...
  onSwitchMethodRjesenje(bool newValue) {
    setState(() {
      if (newValue == true) {
        appState.rjesenjeShownTemperatura = true;
      } else {
        appState.rjesenjeShownTemperatura = false;
      }
    });
  }
    ...
    return Padding(
      ...
      child: ListView(
        children: [
          ...
          buildSwitchOption('Postupak', appState.postupakShownTemperatura,
          ...
          buildSwitchOption('Rješenje', appState.rjesenjeShownTemperatura,
          ...
          Padding(
            ...
            child: Text(
              'Težina zadatka: ${appState.currentSliderValueTemperatura.round()}',
              style: TextStyle(
          ...
          Padding(
            ...
            child: SliderTheme(
              ...
              child: Slider(
                min: 1,
                max: 4,
                value: appState.currentSliderValueTemperatura,
                divisions: 3,
                label: '${appState.currentSliderValueTemperatura.round()}',
                onChanged: (value) {
                  setState(() {
                    appState.currentSliderValueTemperatura = value;
                  });
                },
              ),
            ),
          ...
          Padding(
            padding: EdgeInsets.symmetric(
                horizontal: screenWidth / 43, vertical: screenHeight / 300),
            child: const ZadatciButtonTemperatura(),
          )
        ],
      ),
    );
