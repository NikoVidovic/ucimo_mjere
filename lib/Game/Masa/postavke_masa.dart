...
import 'package:hello_world/Game/Masa/masa_zadatci_button.dart';
import 'package:provider/provider.dart';
import '../../app_state.dart';

class PostavkeMasa extends StatefulWidget {
  ...
  @override
  State<PostavkeMasa> createState() => _PostavkeMasaState();
}

class _PostavkeMasaState extends State<PostavkeMasa> {
  ...
  onSwitchMethodRjesenje(bool newValue) {
    setState(() {
      if (newValue == true) {
        appState.rjesenjeShownMasa = true;
      } else {
        appState.rjesenjeShownMasa = false;
      }
    });
  }
    ...
    return Padding(
      ...
      child: ListView(
        children: [
          ...
          buildSwitchOption(
              'Postupak', appState.postupakShownMasa, onSwitchMethodPostupak),
          SizedBox(
          ...
          buildSwitchOption(
              'Rješenje', appState.rjesenjeShownMasa, onSwitchMethodRjesenje),
          SizedBox(
            height: screenHeight / 30,
          ),
          Padding(
            ...
            child: Text(
              'Težina zadatka: ${appState.currentSliderValueMasa.round()}',
              style: TextStyle(
          ...
          Padding(
            ...
            child: SliderTheme(
              ...
              child: Slider(
                min: 1,
                max: 4,
                value: appState.currentSliderValueMasa,
                divisions: 3,
                label: '${appState.currentSliderValueMasa.round()}',
                onChanged: (value) {
                  setState(() {
                    appState.currentSliderValueMasa = value;
                  });
                },
              ),
            ),
